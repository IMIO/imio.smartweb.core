...
function TaxonomyFilter({ onChange, sub, title, token, isActive, setCat, onClick }) {
    ...
    const [filterValue, setFilterValue] = useState({taxonomy_contact_category_for_filtering: null});
    ...
    const handleApply = (value,title) => {
        ...
        if (!secondLevelVisible) {
            onChange({value:value,label:title},{name: "taxonomy_contact_category_for_filtering"});
        } else {
    ...
    return (
        <div onClick={onClick}>
            <div className={secondLevelVisible ? "dropDownFilter dropDownFilter-active" : "dropDownFilter"}>
                {/* Lien avec gestionnaire d'événements */}
                <a className="sub0" href="#" value={title}  
                    onClick={(e) => {
                        e.preventDefault();
                        handleLinkClick();
                ...
                {firstLevelOptions && (
                    <div className={secondLevelVisible ? "sub1 dropDownFilter-visible" : "sub1 dropDownFilter-invisble"}>
                        <Select
                ...
                {secondLevelOptions && (
                    <div className={thirdLevelVisible ? "sub2 dropDownFilter-visible" : "sub2 dropDownFilter-invisble"}>
                        <Select
                            name={"taxonomy_contact_category_for_filtering"}
                            className="select-custom-class library-facilities"
